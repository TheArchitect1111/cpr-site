...
export const GUIDED_PLATFORMS: Record<EAPlatformId, GuidedPlatformConfig> = {
  ...
  cpr: {
    ...
    nextActions: [
      { id: 'profile', label: 'Update profile', description: 'Complete athlete details' },
      { id: 'timeline', label: 'View recruiting timeline', description: 'Know your next milestone' },
      { id: 'messages', label: 'Open messages', description: 'Stay connected with CPR' },
    ],
...
export function gfsStorageKey(platform: EAPlatformId, scope: string): string {
  return `ea-gfs-v1-${platform}-${scope.replace(/\//g, '-')}`;
}
...
export function buildCprActionCenter(input: {
  base: string;
  onboarding: {
  ...
  if (!input.onboarding.profileComplete) {
    items.push({
      id: 'profile',
      title: 'Complete athlete profile',
      detail: 'Coaches need your basics before outreach can begin.',
      priority: 'critical',
      href: `${input.base}/recruiting-timeline`,
      cta: 'Get started',
    });
  }

  if (!input.onboarding.videoUploaded) {
    items.push({
      id: 'video',
      title: 'Upload highlight video',
      detail: 'Your best footage belongs in the document vault.',
      priority: 'high',
      href: `${input.base}/document-vault`,
      cta: 'Upload',
    });
  }

  if (!input.onboarding.assessmentComplete) {
    items.push({
      id: 'assessment',
      title: 'Finish recruiting assessment',
      detail: 'Unlock personalized school and coach recommendations.',
      priority: 'high',
      href: `${input.base}/ask-cpr`,
      cta: 'Start',
    });
  }
  ...
  if (input.opportunityCount > 0) {
    items.push({
      id: 'opportunities',
      title: 'School interest needs review',
      detail: `${input.opportunityCount} program(s) on your radar — review next steps.`,
      priority: 'medium',
      href: `${input.base}/scholarship-center`,
      cta: 'Review',
    });
  }
  ...
  items.push({
    id: 'timeline',
    title: 'Check recruiting timeline',
    detail: 'See what deserves attention this month.',
    priority: 'medium',
    href: `${input.base}/recruiting-timeline`,
    cta: 'View timeline',
  });
