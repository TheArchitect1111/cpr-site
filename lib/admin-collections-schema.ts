...
export const COLLECTIONS: Record<CollectionId, CollectionDef> = {
  ...
  'site-events': {
    ...
    fields: [
      ...
      { key: 'registrationMode', label: 'Registration method', type: 'select', options: ['Native', 'External'], hint: 'Native uses the CPR registration form. External sends families to the registration link.' },
      { key: 'paymentUrl', label: 'Payment link', type: 'text', placeholder: 'https://', hint: 'Optional. Native registrations are sent here after the form is completed.' },
      { key: 'waiverText', label: 'Waiver acknowledgment', type: 'textarea', hint: 'Use organization-approved language. The standard acknowledgment is used when blank.' },
  ...
  'camp-registrations': {
    ...
    fields: [
      ...
      { key: 'campId', label: 'Camp ID', type: 'text' },
      { key: 'campName', label: 'Camp', type: 'text', required: true },
      { key: 'parentGuardianName', label: 'Parent or guardian', type: 'text', required: true },
      ...
      { key: 'waiverAcceptedAt', label: 'Waiver accepted', type: 'text' },
